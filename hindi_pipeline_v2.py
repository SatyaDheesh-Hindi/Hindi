"""
Satya Hindi Translation Pipeline v2.

Replaces the Qwen-14B masala/critic/fallback design with:
  NLLB (faithful NMT) -> glossary substitution -> verification gates.
...
DB access, sharding, resume logic and has_more looping mirror v1 (and include
the fatal-bug fixes: shard-filtered completed-checks, failure tracking).
"""
import os
import sys
import argparse
...
def process_articles(translator, shard, num_shards, batch_size):
    ...
    for idx, (article_id, eng_headline, comp) in enumerate(chunk_rows):
        try:
            ...
            # 3. Verify BEFORE glossary (gate on the raw Hindi vs English source)
            ok_body, rb = core.verify(eng_summary, hi_body_raw)
            ok_title, rt = core.verify(eng_headline or "", hi_title_raw)
            if not ok_body:
...
# ==============================================================================
def main():
    ap = argparse.ArgumentParser(description="Satya Hindi Translation v2 (NLLB)")
